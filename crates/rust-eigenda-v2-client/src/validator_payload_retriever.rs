...
use crate::{
    commitment_utils::generate_and_compare_blob_commitment, errors::ValidatorPayloadRetrieverError,
...
impl ValidatorPayloadRetriver {
    ...
    pub async fn get_payload(
        ...
        for _ in blob_header.quorum_numbers.clone() {
            let payload = self
                .get_payload_from_quorum(
                    blob_header,
                    eigenda_cert.batch_header.reference_block_number,
                )
                .await;
    ...
    async fn get_payload_from_quorum(
        ...
        let blob: Blob = self
            .retrieve_blob_with_timeout(blob_header, reference_block_number)
            .await?;

        let valid = generate_and_compare_blob_commitment(
            self.g1_srs.clone(),
            blob.serialize(),
            blob_header.commitment.commitment,
        )?;
    ...
    async fn retrieve_blob_with_timeout(
        ...
        let blob_bytes = timeout(
            ...
            self.retrieval_client
                .get_blob(blob_header, reference_block_number),
        )
        .await
        .map_err(|_| ValidatorPayloadRetrieverError::Timeout)??;
